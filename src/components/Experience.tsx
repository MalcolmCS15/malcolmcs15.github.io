...
const Experience: React.FC = () => {
  ...
  // Deep-link support: /experience?highlight=<company> expands that entry,
  // scrolls to it, and pulses a highlight for a couple of seconds.
  ...
  const sorted = useMemo(() => {
    return experienceTimeline
      // An explicit isCurrent: false in an experience/*.md file keeps an ongoing entry
      // out of the "Present" group (it groups under its end/start year instead)
      .map(e => ({ ...e, isCurrent: e.isCurrent === false ? false : (!e.end || e.end.toLowerCase() === 'present') }))
      .sort((a, b) => {
  ...
  const grouped = useMemo(() => {
    ...
    for (const e of filtered) {
      const key = e.isCurrent ? 'Present' : new Date(e.end ?? e.start).getFullYear().toString()
  ...
  useEffect(() => {
    ...
    if (!target) return
    const match = sorted.find(e => e.company === target)
    if (!match) return
    const id = `${match.title}-${match.company}-${match.start}`
    setExpandedItems(prev => ({ ...prev, [id]: true }))
    setHighlightedId(id)
    const scrollTimer = setTimeout(() => {
      entryRefs.current[id]?.scrollIntoView({ behavior: 'smooth', block: 'center' })
    }, 150)
    const fadeTimer = setTimeout(() => setHighlightedId(null), 2500)
    return () => { clearTimeout(scrollTimer); clearTimeout(fadeTimer) }
  }, [location.search, sorted])
  ...
  const education = experienceData.education.courses.filter(c => !c.bioOnly)
  ...
  const handleCommand = (cmd: string) => {
    ...
    switch (parts[0]) {
      ...
      case 'whoami': out([siteOwner.name.full, 'AI/ML engineer · athlete · fun']); break
      case 'help': out([
        '"filter [all|academic|industry]"  "clear"  "whoami"',
        '"sudo hire-me"  "cat skills"  "dogs"',
      ]); break
      case 'sudo':
        if (parts.slice(1).join(' ') === 'hire-me')
          out([
            `Email: ${siteOwner.contact.academicEmail}`,
            "Status: Open to opportunities!",
            "initiating hire sequence...",
            `Thank you for hiring ${siteOwner.name.full}, he will appear on your payroll promptly`,
      ...
      case 'cat':
        if (parts[1] === 'skills') out([
          siteOwner.skills.map(s => s.name).join(' · '),
        ])
        else out([`cat: ${parts[1] || ''}: not found`])
        break
      case 'dogs': out(['Sukey, Hoosier <3']); break
      ...
      <VStack spacing={6} maxW="1400px" mx="auto" px={[2, 4, 6]}>
        ...
        >
          ...
          <Flex
            ...
            align="center"
          >
            {(['all', 'industry', 'academic'] as FilterType[]).map(f => {
              ...
              <Box key={group.year}>
                ...
                {group.items.map(exp => {
                  ...
                  return (
                    <Box
                      key={id}
                      ref={el => { entryRefs.current[id] = el }}
                      borderBottom={`1px solid ${isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.04)'}`}
                      bg={isHighlighted ? 'rgba(252, 90, 3, 0.10)' : undefined}
                      boxShadow={isHighlighted ? 'inset 3px 0 0 #fc5a03' : undefined}
                      _hover={{ bg: isHighlighted ? undefined : hoverBg }}
