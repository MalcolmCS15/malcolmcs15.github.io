...
/** Render text with `\n` line breaks as separate lines (each still supporting **bold**) */
const renderMultiline = (text: string, color: string, boldColor: string) =>
  text.split('\n').map((line, i) => (
    <Text as="span" key={i} display="block" color={color}>
              ...
              <Flex key={index} gap={[3, 4]} align="start" py={3} position="relative">
                ...
                <Box flex={1} pb={2}>
                  ...
                  <Text fontSize="xs" lineHeight="tall" mb={2}>
                    {renderMultiline(phase.description, textColor, boldColor)}
                  </Text>
                  {phase.tags && (
