...
const iconFor = (a: Award): string => {
  // Per-award override: set `"icon": "FaWhatever"` on an entry in awards.json
...
const AwardRow = ({ award }: { award: Award }) => {
  ...
  const override = award.iconColor
  const kindColor = useColorModeValue(
    override ? (Array.isArray(override) ? override[0] : override) : meta.color[0],
    override ? (Array.isArray(override) ? override[1] : override) : meta.color[1],
  )
  ...
  const content = (
    ...
    >
      ...
      <VStack spacing={0} align="end" flexShrink={0}>
        {/* Dash- or comma-separated years ("2025 – 2026", "2021, 2022, 2023")
            render stacked, one year per line */}
        {award.date.split(/\s*[–,-]\s*/).map((part, i) => (
          <Text key={i} fontSize="2xs" fontFamily="mono" color={mutedColor} whiteSpace="nowrap">
            {part}
          </Text>
        ))}
      </VStack>
...
const AccomplishmentsTerminal: React.FC = () => {
  ...
  return (
    <Container maxW={["full", "full", "7xl"]} px={[2, 4, 8]}>
      <Heading size={["sm", "md"]} mb={4}>{t('about.awardsAndHonors')}</Heading>
