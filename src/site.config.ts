...
// To add another language (e.g. Spanish), import '@content/es/site.json'
// and register it in getLocalizedSiteConfig below.
...
/** Get site config for a given language. Accepts region-suffixed codes (es-MX, ...). */
 ...
 *  The special platform "email" resolves to a mailto: link from contact.email.
 *  Icons whose URL is not configured are hidden. */
export const heroSocialIcons = (siteConfig.heroSocialIcons ?? []).map(item => ({
  icon: item.icon,
  label: item.label,
  color: item.color,
  href: item.platform === 'email'
    ? `mailto:${siteConfig.contact.email}`
    : (siteConfig.social as Record<string, string>)[item.platform] ?? '',
