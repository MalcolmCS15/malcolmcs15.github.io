...
├── education.json        ← Education, timeline entries, reviewing
...
server.tool(
  "write_json_content",
  "Write or update a JSON content file (site.json, education.json, news.json, awards.json, research.json, logos.json). " +
...
server.tool(
  "add_experience",
  "Add a timeline entry to education.json. " +
  ...
  async (params) => {
    ...
    const written: string[] = [];
    for (const lang of targets) {
      const expPath = path.join(getContentDir(lang), "education.json");
...
server.tool(
  "add_education",
  "Add an education entry to education.json. " +
    "Use language='zh' for Chinese, or language='both' to add the same entry to both EN and ZH.",
  {
...
server.tool(
  ...
  async () => {
    ...
    function countContent(lang: Language) {
      ...
      const expPath = path.join(contentDir, "education.json");
  ...
  async ({ confirm }) => {
    ...
    for (const lang of ["en", "zh"] as Language[]) {
      ...
      if (lang === "en" || fs.existsSync(contentDir)) {
        ...
        writeJson(path.join(contentDir, "education.json"), {
          education: { courses: [] },
          reviewing: [],
          timeline: [],
        });
